...
export default function CourseList() {
  ...
  const loadTeachers = async () => {
    try {
      const res = await teacherApi.page({ page: 1, size: 500 });
  ...
  const teacherOptions = teachers.map((t) => ({ label: `${t.teacherId} ${t.name}`, value: t.id }));
      ...
      <Form layout="inline" onFinish={onSearch} style={{ marginBottom: 16 }}>
        ...
        <Form.Item>
          <Space>
            ...
            {canEdit && <Button type="primary" onClick={openAdd}>新增课程</Button>}
          </Space>
        </Form.Item>
      </Form>
      <Table
      ...
      {canEdit && (
        <Modal title={editingId ? '编辑课程' : '新增课程'} open={modalOpen} onOk={onOk} onCancel={() => setModalOpen(false)} width={600} destroyOnClose>
          <Form form={form} layout="vertical" style={{ marginTop: 16 }}>
            ...
            <Form.Item name="teacherId" label="授课教师"><Select allowClear options={teacherOptions} showSearch optionFilterProp="label" /></Form.Item>
